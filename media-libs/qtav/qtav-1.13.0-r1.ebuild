...
https://dev.gentoo.org/~johu/distfiles/${PN}-1.12.0-capi.h-${CAPI_HASH}.xz"
...
PATCHES=(
	"${FILESDIR}"/${PN}-1.12.0-installpaths.patch
	"${FILESDIR}"/${PN}-1.12.0-qt5.14.patch
	"${FILESDIR}"/${PN}-1.12.0-qt5.15.patch
)

src_prepare() {
	cmake_src_prepare
	cp "${WORKDIR}/${PN}-1.12.0-capi.h-${CAPI_HASH}" contrib/capi/capi.h \
