...
src_prepare() {
	...
	/bin/sh -x "${S}/unpack/Unix/Installer/MathInstaller" -auto "-targetdir=${S}/${M_TARGET}" "-execdir=${S}/opt/bin" || die
...
src_install() {
	...
	domenu "${FILESDIR}/wolfram-${PN}.desktop"
	for ICON in 32 64 128
	do
		newicon --size "${ICON}" "${D}/opt/Wolfram/Mathematica/12.2/SystemFiles/FrontEnd/SystemResources/X/App-${ICON}.png" "wolfram-${PN}.png"
	done
	# install mime types
	...
	find "${D}/opt/Wolfram/Mathematica/12.2/Executables" -type l -delete
	dosym ../"SystemFiles/Kernel/Binaries/Linux-x86-64/wolframscript" "/opt/Wolfram/Mathematica/12.2/Executables/wolframscript"
}
