...
PATCHES=(
	"${FILESDIR}"/${PN}-02.19b-gentoo.patch
)
...
src_compile() {
	...
	emake VERSION="${PV}" SQLITE="${sqlite}" all
	use gtk && emake VERSION="${PV}" SQLITE=${sqlite} gui
}

src_install() {
	emake VERSION="${PV}" DESTDIR="${D}" PREFIX="${EPREFIX}/usr" install $(usex gtk 'install-gui' '')
